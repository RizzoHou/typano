...
- `--check-layout` — the key maps. Asserts FreePiano's anchor pitches, that every row of every drawn keyboard sums to one width, that the four diatonic rows stack in octaves column by column, that the right-hand run repeats no note, and that no control key was swallowed by a lengthened note row.
...
**The drawn keyboard is not verifiable either.** `screencapture` over ssh dies with `could not create image from display` — the ssh session has no Screen Recording grant — so which editing keys sit in a 98's right-hand column is a guess. Vendors disagree there, and it is cosmetic: mappings are keyed by virtual key code, so a board drawn wrong still plays right.
...
- **On the diatonic layout almost the whole board is a note** — Return, Backspace, Tab, the digits, the brackets, both Shifts. Esc is a second sustain latch there, because a 98 usually ends its bottom row Alt / Fn with no right ⌘ for the latch to live on. F5–F12 carry octave and velocity, and reach the app only when macOS is sending real function keys — otherwise they are media keys and never arrive as `keyDown`.
